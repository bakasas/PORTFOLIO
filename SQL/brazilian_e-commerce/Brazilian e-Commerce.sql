...
SELECT c.["geolocation_state"]
	  ,COUNT(c.["geolocation_state"]) AS ["customer_count"]
FROM 
	(
...
SELECT c.["geolocation_city"]
	  ,COUNT(c.["geolocation_city"]) AS ["customer_count_purchased"]
FROM 
	(
	SELECT DISTINCT b.["geolocation_city"]
		  ,a.["customer_id"]
	FROM [Brazilian e-Commerce].[dbo].[customers_clean] AS a
	INNER JOIN [Brazilian e-Commerce].[dbo].[geolocations_clean] AS b
	  ON a.["customer_zip_code_prefix"] = b.["geolocation_zip_code_prefix"]
...
INNER JOIN [Brazilian e-Commerce].[dbo].[orders_clean]  AS b 
...
-- list of Sellers who have made 2 or more sales in 7 days
...
SELECT c.["geolocation_state"]
	  ,COUNT(c.["geolocation_state"]) AS ["seller_count"]
FROM 
	(
	SELECT DISTINCT b.["geolocation_state"]
		  ,a.["seller_id"]
...
SELECT RANK() OVER(ORDER BY ["seller_total_revenue"] DESC) AS ["rank"]
	  ,["seller_id"]
	  ,ROUND(["seller_total_revenue"], 0) AS ["seller_total_revenue"]
...
-- list of average Freight values in each City per State
